...
class RenderFont(object):
    ...
    def __init__(self, LANG, data_dir='data'):
        ...
        self.f_shrink = 0.90
        self.max_shrink_trials = 7 # 0.9^5 ~= 0.6
        # the minimum number of characters that should fit in a mask
        # to define the maximum font height.
        self.min_nchar = 2
        self.min_font_h = 12 #px : 0.6*12 ~ 7px <= actual minimum height
        self.max_font_h = 80 #px
        self.p_flat = 0.10
        ...
        self.text_source = TextSource(min_nchar=self.min_nchar,
                                      fn=osp.join(data_dir,'newsgroup/newsgroup_'+LANG+'.txt'))

        # get font-state object:
        self.font_state = FontState( data_dir=data_dir, LANG = LANG)
...
#         print(rect.centery, curve, mid_idx)
    ...
    def place_text(self, text_arrs, back_arr, bbs, seed = None):
        if seed is not None:
            rng = np.random.RandomState(233*seed)
        else:
            rng = np.random.RandomState(None)
        areas = [-np.prod(ta.shape) for ta in text_arrs] # space that text would occupy
        order = np.argsort(areas) # put text from largest to smallest
...
#             if len(np.nonzero(ba<127)[0]) > np.prod(ta.shape) * 10: # when background is much larger than text array

            # find valid location for text
            if seed is not None:
                temp_rng = np.random.RandomState(seed+i+666)
            else:
...
#             print(safemask.shape, ba.shape, loc)
            ...
            dist = minloc - np.array([[loc[0], loc[1]]])
    ...
    def sample_font_height_px(self,h_min,h_max,seed=None):
        if seed is None:
            ...
            rng = np.random.RandomState(88*seed)
        if rng.rand() < self.p_flat:
            rnd = rng.rand()
        else:
            rnd = rng.beta(2.0,2.0)
    ...
    def render_sample(self,font,mask, seed=None): # mask is the collision mask directly from the depth.h5
        """
...
#         if f_asp < 1e-3:
#             print('Warning: asp = 0, reset to 1')
#             f_asp = 1.0
        f_asp = 1.0
        ...
        while i < self.max_shrink_trials and max_font_h > self.min_font_h:
            ...
            f_h_px = self.sample_font_height_px(self.min_font_h, max_font_h, seed)
            ...
            # update for the loop
            max_font_h = 0.9*max_font_h #f_h_px 
            i += 1

            font.size = f_h # set the font-size

            # compute the max-number of lines/chars-per-line:
            # On the basis of the font height, width and the width dimensions get the nline, nchar

            nline, nchar = self.get_nline_nchar(mask.shape[:2],f_h,f_h*f_asp)
            # assert nline >= 1 and nchar >= self.min_nchar
            if nline < 1 or nchar < self.min_nchar:
                continue
            # sample text:
#             text_type = sample_weighted(self.p_text)
            text_type = "WORD"
            # samples the text to be placed #marker1
            text = self.text_source.sample(nline,nchar,text_type)
            while len(text)==0 or np.any([len(line)==0 for line in text]) or self.has_number( text ): # len(text)<nchar/2 or 
                text = self.text_source.sample(nline,nchar,text_type)

            num_ext = (nchar - len(text))//2
...
#         print(u'Fail to render {}'.format(text))
        return
...
class FontState(object):
    ...
    def __init__(self, LANG ,data_dir='data'):
        ...
        font_model_path = osp.join(data_dir, 'models/font_px2pt_{}.cp'.format(LANG))
        ...
        self.FONT_LIST = osp.join(data_dir, 'fonts/fontlist_'+LANG+'.txt')
    ...
    def sample(self, seed = None):
        ...
        """
        rng_random = np.random.RandomState(None)
        if seed is None:
        ...
        return {
            ...
            'strong': False,
            'oblique': False,
            'strength': (self.strength[1] - self.strength[0])*rng_fixed.rand() + self.strength[0],
            'char_spacing': int(self.kerning[3]*(rng_fixed.beta(self.kerning[0], self.kerning[1])) + self.kerning[2]),
            'border': False,
            'random_caps': False,
            'capsmode': (self.capsmode[0]), #random.choice
            'curved': False,
...
class TextSource(object):
    ...
    def sample_word(self,nline_max,nchar_max,niter=100):
        ...
        words = []
        while len(words) <= 0:
            rand_line = self.txt[rng.choice(len(self.txt))]
            words = rand_line.split()
        rand_word = rng.choice(words)
        ...
        if not self.is_good([rand_word])[0]:
            return []
        else:
            return rand_word[0:nchar_max]
    ...
    def sample_para(self,nline_max,nchar_max):
        ...
        if lines is not None:
            ...
            return []
#         else: # when seed is not None, generate test samples
#             # sample a random font-height:
#             f_h_px = self.sample_font_height_px(self.min_font_h, max_font_h, seed = seed)
# #             print(f_h_px)
#             #print "font-height : %.2f (min: %.2f, max: %.2f)"%(f_h_px, self.min_font_h,max_font_h)
#             # convert from pixel-height to font-point-size:
#             f_h = self.font_state.get_font_size(font, f_h_px)
...
#             # samples the text to be placed #marker1
#             text = ''
#             while len(text)==0 or np.any([len(line)==0 for line in text]):
#                 text = self.text_source.sample(nline,nchar,text_type) # get content of text, nchar: maximum number of characters can render
#             # render the text:
