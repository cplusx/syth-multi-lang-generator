...
def predict_on_rex_models(img_file):
    import cv2
    import urllib, cStringIO
    from PIL import Image
    from keras.applications.vgg16 import preprocess_input
    ...
    img = image_resize(img, width=540, height = None)
...
for each_directory in dir_list_inf:
    dir_unf_full_path = os.path.join(dir_unfiltered_images, each_directory)
    create_directory(os.path.join(tgt_filtered_path, each_directory))
    dir_filtered_path = os.path.join(tgt_filtered_path, each_directory)
    for each_file in os.listdir(dir_unf_full_path):
        img_full_path = os.path.join(dir_unf_full_path, each_file)
        tgt_img_path = os.path.join(dir_filtered_path, each_file)
        # for already processed images
        if (os.path.exists(tgt_img_path)):
#	    print("passing for ", tgt_img_path)
            continue
        model0_mask = predict_on_rex_models(img_full_path)
        sum_ = sum(model0_mask.ravel())

        if(sum_ <= 20):
#            print('copying from = ', img_full_path, "to = ",tgt_img_path)
