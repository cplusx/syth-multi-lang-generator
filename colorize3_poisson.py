'''
This file realize the function of colorizing text array
'''
import cv2 as cv
import numpy as np 
...
class FontColor(object):
    ...
    def sample_from_seed( self, seed = None ):
        if seed is None:
            rng = np.random.RandomState( None )
            # print('Using random seed')
        else:
            rng = np.random.RandomState( 321*seed )
            # print('Using fixed seed')
        nn = rng.randint(0, self.ncol)
        data_col = self.colorsRGB[np.mod(nn,self.ncol),:]

        col1 = self.sample_normal(data_col[:3],data_col[3:6], seed)
        col2 = self.sample_normal(data_col[6:9],data_col[9:12], seed)

        return (col2, col1)


    def sample_normal(self, col_mean, col_std, seed):
        """
        ...
        if seed is None:
            ...
            rng = np.random.RandomState( 321*seed )
            # print('Using fixed seed')
        col_sample = col_mean + col_std * rng.randn()
        return np.clip(col_sample, 0, 255).astype('uint8')

    def sample_from_data(self, bg_mat, seed = None):
        """
...
class Colorize(object):
    ...
    def color_border(self, col_text, col_bg, seed = None):
        """
        ...
        if seed is None:
            ...
            rng = np.random.RandomState( 111*seed )
        ...
        def get_sample(x):
            ps = np.abs(vs - x/255.0)
            ps /= np.sum(ps)
            v_rand = np.clip(rng.choice(vs,p=ps) + 0.1*rng.randn(),0,1)
            return 255*v_rand
    ...
    def color_text(self, text_arr, h, bg_arr, seed = None):
        """
    ...
    def process(self, text_arr, bg_arr, min_h, seed = None):
        ...
        # decide on a color for the text:
        l_text, fg_col, bg_col = self.color_text(text_arr, min_h, bg_arr, seed)
        bg_col = np.mean(np.mean(bg_arr,axis=0),axis=0)
        l_bg = Layer(alpha=255*np.ones_like(text_arr,'uint8'),color=bg_col)

        l_text.alpha = l_text.alpha * np.clip(0.88 + 0.1*rng.randn(), 0.72, 1.0)
        layers = [l_text]
        blends = []

        
        # add border:
        if rng.rand() < self.p_border:
            if min_h <= 15 : bsz = 1
            elif 15 < min_h < 30: bsz = 3
            else: bsz = 5
            border_a = self.border(l_text.alpha, size=bsz)
            l_border = Layer(border_a, self.color_border(l_text.color,l_bg.color, seed))
            layers.append(l_border)
            blends.append('normal')

        # add shadow:
        if rng.rand() < self.p_drop_shadow:
            ...
            # shadow angle:
            theta = np.pi/4 * rng.choice([1,3,5,7]) + 0.5*rng.randn()

            # shadow shift:
            if min_h <= 15 : shift = 2
            elif 15 < min_h < 30: shift = 7+rng.randn()
            else: shift = 15 + 3*rng.randn()

            # opacity:
            op = 0.80 + 0.1*rng.randn()
    ...
    def color(self, bg_arr, text_arr, hs, place_order=None, pad=20, seed = None):
        ...
        for i in place_order[::-1]:
            ...
            rdr0 = self.process(text_patch, bg, hs[i], seed)
