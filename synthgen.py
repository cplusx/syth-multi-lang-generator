...
from store_the_results import SavingResults
...
class RendererV3(object):
    ...
    def feather(self, text_mask, min_h, seed = None): 
        if seed is None:
            rng = np.random.RandomState(None)
        else:
            rng = np.random.RandomState(222*seed)
        # determine the gaussian-blur std:
        if min_h <= 15 :
            bsz = 0.25
            ksz=1
        elif 15 < min_h < 30:
            bsz = max(0.30, 0.5 + 0.1*rng.randn())
            ksz = 3
        else:
            bsz = max(0.5, 1.5 + 0.5*rng.randn())
            ksz = 5
        return cv2.GaussianBlur(text_mask,(ksz,ksz),bsz)

    def place_text(self,rgb,collision_mask,H,Hinv, lang, font_temp, seed = None):
        '''
        rgb: image
        collision_mask: place of mask to place text
        H, Hinv: homography and its inverse
        lang: language
        font_temp: None
        seed: control random process
        '''
        if not font_temp:
            font_temp = self.text_renderer.font_state.sample(seed = seed)

        font = self.text_renderer.font_state.init_font(font_temp)

        render_res = self.text_renderer.render_sample(font,collision_mask, seed=seed)
...
#             return #None
        ...
        #feathering:
        text_mask = self.feather(text_mask, min_h, seed)

        im_final = self.colorizer.color(rgb,[text_mask],np.array([min_h]), seed=seed)

        # self.store(bb, text)

        return im_final, text, bb, collision_mask, font_temp
...
#         else:
    ...
    def render_text(self, imname,rgb,depth,seg,area,label, font_temp, lang, ninstance=1, viz=False, seed = None):
        # self.srObj.set_image(rgb)
        """
        ...
        """
        np.random.seed( seed ) # set seed manually to control location of text rendered
        try:
            # depth -> xyz
            ...
            while True and trial < max_trial:
                ...
                for idx in reg_range:
                    ...
                    try:
                        if self.max_time is None:
                            txt_render_res = self.place_text(img,place_masks[ireg],
                                                             ...
                                                             regions['homography_inv'][ireg], lang, font_temp, seed = seed_)
                        else:
                    ...
                    if txt_render_res is not None:
                        ...
                        itext.append(text)
                        ibb.append(bb)
                if len(itext) > best_render:
                    best_render = len(itext)
                    itext_best = itext
                    ibb_best = ibb
                    best_img = img.copy()
                if len(itext) == m:
                    ...
                    print('Expect {} regions, only {} region rendered, retry'.format(m, len(itext) ))
                    continue


#                     print("=============================================================")
#                     print("placing the image for = ", imname)
#                     print("placing this text = ", text)
#                     print("Here are the font configurations = ", font)
#                     print("=============================================================")
